 ...
 * @return void* : Pointer to allocated memory address (if successful).
 */
void *mem_alloc(size_t size);

/**
 * @brief Re-allocate the previously allocated block in [p],
 * making the new block [size] bytes long.
 * 
 * @param ptr   : Pointer to memory address to be reallocated.
 * @param size  : Number of bytes to allocate.
 * @return void* : Pointer to reallocated memory address (if successful).
 */
